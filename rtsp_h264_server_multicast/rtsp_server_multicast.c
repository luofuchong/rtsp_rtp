...
static int handleCmd_DESCRIBE(char* result, int cseq, char* url)
{
    ...
    sprintf(sdp, "v=0\r\n"
                 ...
                 "a=control:track0\r\n",
                 time(NULL), localIp, MULTICAST_PORT, MULTICAST_IP);
...
void* sendRtpPacket(void* arg)
{
    ...
    char* frame = malloc(MAX_FRAME_SIZE);
    struct RtpPacket* rtpPacket = (struct RtpPacket*)malloc(RTP_HEADER_SIZE+RTP_MAX_PKT_SIZE+2);
    ...
    while(1)
    {
        frameSize = getFrameFromH264File(fd, frame, MAX_FRAME_SIZE);
        ...
        ret = rtpSendH264Frame(sockfd, MULTICAST_IP, MULTICAST_PORT,
                            rtpPacket, frame+startCode, frameSize-startCode);
        if(ret > 0) {
...
void* doClientWorker(void* arg)
{
    ClientParam *pClient = (ClientParam *)arg;
...
static void DoClient(int clientSockfd, char* clientIp, int clientPort)
{
    ClientParam *pClient = malloc(sizeof(ClientParam));

    pClient->Sockfd = clientSockfd;
    pClient->Ip = strdup(clientIp);
    pClient->Port = clientPort;

    pthread_create(&pClient->ThreadId, NULL, doClientWorker, (void *)pClient); 
}
...
int main(int argc, char* argv[])
{
    ...
    while(1)
    {
        ...
        printf("accept client:%d;client ip:%s,client port:%d\n", clientSockfd, clientIp, clientPort);
