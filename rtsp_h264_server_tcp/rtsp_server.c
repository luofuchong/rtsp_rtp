...
static void doClient(int clientSockfd, const char* clientIP, int clientPort)
{
    ...
    while(1)
    {
        ...
        if(!strcmp(method, "PLAY"))
        {
            ...
            struct RtpPacket* rtpPacket = (struct RtpPacket*)malloc(RTP_HEADER_SIZE+4+RTP_MAX_PKT_SIZE+2);
            ...
            while (1)
            {
                ...
                ret = rtpSendH264Frame(clientSockfd, rtpChannel, rtpPacket, frame+startCode, frameSize-startCode);
                if(ret < 0) {
                    break;
                } else if(ret > 0) {
