...
#define CLIENT_IP       "239.255.255.11"
...
static int getFrameFromH264File(int fd, char* frame, int size)
{
    ...
    if(!nextStartCode)
    {
        lseek(fd, 0, SEEK_SET); //最后一个nalu，重置
        frameSize = rSize;
    }
    else
    {
        frameSize = (nextStartCode-frame);
        lseek(fd, frameSize-rSize, SEEK_CUR); //定位到下一个nalu的开始位置
    }
                            ...
                            struct RtpPacket* rtpPacket, uint8_t* frame, uint32_t frameSize)
{
    ...
    rtpPacket->rtpHeader.timestamp += 90000/FPS; //sihid
...
int main(int argc, char* argv[])
{
    ...
    rtpPacket = (struct RtpPacket*)malloc(RTP_HEADER_SIZE+RTP_MAX_PKT_SIZE+2);
    ...
    while(1)
    {
        ...
        ret = rtpSendH264Frame(socket, CLIENT_IP, CLIENT_PORT,
